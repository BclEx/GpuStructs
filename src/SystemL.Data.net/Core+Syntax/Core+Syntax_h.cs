...
const int N_COLCACHE = 10;
//#define BMS ((int)(sizeof(Bitmask)*8))
#if !MAX_EXPR_DEPTH
const int MAX_EXPR_DEPTH = 1000;
#endif
//#ifdef OMIT_TEMPDB
...
//    INT_TO_PTR(arg), 0, 0, xStep,xFinal,#zName,0,0}
    ...
    public class IIndexInfo
    {
        ...
        // INPUTS
        public Constraint[] Constraints; // Table of WHERE clause constraints
        public int ConstraintsLength;
        public Orderby[] OrderBys; // The ORDER BY clause
...
#region Select
	...
	public class SrcList
	{
		public class SrcListItem
		{
			...
			string Alias;		// The "B" part of a "A AS B" phrase.  zName is the "A"
			...
			bool NotIndexed;    // True if there is a NOT INDEXED clause
			bool IsCorrelated;  // True if sub-query is correlated
			bool ViaCoroutine;  // Implemented as a co-routine
#if !OMIT_EXPLAIN
			byte SelectId;     // If pSelect!=0, the id of the sub-select in EQP
#endif
		...
		public class _u
		{
			public class _in
			{
				public class InLoop
				{
					...
					byte EndLoopOp;	// IN Loop terminator. OP_Next or OP_Prev
				}
