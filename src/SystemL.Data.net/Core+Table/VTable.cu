...
__device__ static RCt sqlite3_create_module(Context *db, const char *name, const ITableModule *module, void *aux, void (*destroy)(void *))
{
	...
	if (sqlite3HashFind(&db->Modules, name, nameLength))
		rc = RC_MISUSE_BKPT;
	else
	{
		Module *mod = (Module *)SysEx::TagAlloc sqlite3DbMallocRaw(db, sizeof(Module) + nName + 1);
		if (mod)
		{
			...
			_assert(!delMod || delMod == mod);
			if (delMod)
			{
				db->MallocFailed = 1;
				SysEx::TagFree(db, delMod);
			}
		}
	}
