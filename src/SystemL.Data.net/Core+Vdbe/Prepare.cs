...
namespace Core
{
    public partial class Prepare
    {
        static void CorruptSchema(InitData data, string obj, string extra)
        {
            ...
            {
                if (obj == null) obj = "?";
                SysEx.TagStrSet(ref data.ErrMsg, ctx, "malformed database schema (%s)", obj);
                if (extra == null)
                    data.ErrMsg = PrintEx.Mappendf(ctx, data.ErrMsg, "%s - %s", data.ErrMsg, extra);
                data.RC = (ctx.MallocFailed ? RC.NOMEM : SysEx.CORRUPT_BKPT());
            }
        }

        public static bool InitCallback(object init, long argc, object p2, object notUsed1)
        {
            ...
            E.DbClearProperty(ctx, db, DB_Empty);
            if (ctx.MallocFailed)
            {
            ...
            else if (!string.IsNullOrEmpty(argv[2]))
            {
                ...
                Vdbe stmt = null;
#if DEBUG
                int rcp = sqlite3_prepare(ctx, argv[2], -1, ref stmt, null);
#else
                sqlite3_prepare(ctx, argv[2], -1, ref stmt, null);
#endif
                RC rc = ctx.ErrCode;
#if DEBUG
...
error_out:
      ...
      return rc;
        ...
        */
        ...
        int saveSqlFlag,          /* True to copy SQL text into the Vdbe */
        Vdbe pReprepare,          /* VM being reprepared */
        ref Vdbe ppStmt,  /* OUT: A pointer to the prepared statement */
...
"selectid", "order", "from", "detail"
        ...
        */
        static int sqlite3Reprepare(Vdbe p)
        {
            ...
            sqlite3TransferBindings(pNew, (Vdbe)p);
...
*/
        ...
        int nBytes,              /* Length of zSql in bytes. */
        bool saveSqlFlag,         /* True to save SQL text into the Vdbe */
        out Vdbe ppStmt, /* OUT: A pointer to the prepared statement */
