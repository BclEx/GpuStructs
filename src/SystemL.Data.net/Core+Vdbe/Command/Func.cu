...
{
	__device__ static CollSeq *sqlite3GetFuncCollSeq(FuncContext *fctx)
	{
		return fctx->Coll;
	}

	__device__ static void sqlite3SkipAccumulatorLoad(FuncContext *fctx)
	{
		fctx->SkipFlag = true;
	}
	...
	static void *contextMalloc(sqlite3_context *fctx, i64 nByte){
		char *z;
	...
	*/
	static void upperFunc(sqlite3_context *fctx, int argc, sqlite3_value **argv){
		char *z1;
		const char *z2;
		int i, n;
		...
		if( z2 ){
			z1 = contextMalloc(fctx, ((i64)n)+1);
			if( z1 ){
				for(i=0; i<n; i++){
					z1[i] = (char)sqlite3Toupper(z2[i]);
				}
				sqlite3_result_text(fctx, z1, n, sqlite3_free);
			}
		}
	}
	static void lowerFunc(sqlite3_context *fctx, int argc, sqlite3_value **argv){
		char *z1;
		const char *z2;
		int i, n;
		UNUSED_PARAMETER(argc);
	...
	*/
	static void randomBlob(
		...
		sqlite3_value **argv
			...
			p = contextMalloc(fctx, n);
			if( p ){
				sqlite3_randomness(n, p);
				sqlite3_result_blob(fctx, (char*)p, n, sqlite3_free);
			}
	}
	...
	*/
	static void last_insert_rowid(
		...
		sqlite3_value **NotUsed2
			...
			** function. */
			sqlite3_result_int64(fctx, sqlite3_last_insert_rowid(db));
	}
	...
	*/
	static void changes(
		...
		sqlite3_value **NotUsed2
			...
			sqlite3 *db = sqlite3_context_db_handle(fctx);
			UNUSED_PARAMETER2(NotUsed, NotUsed2);
			sqlite3_result_int(fctx, sqlite3_changes(db));
	}
...
# define GlogUpperToLower(A)   if( !((A)&~0x7f) ){ A = sqlite3UpperToLower[A]; }
	...
	static void likeFunc(
		...
		sqlite3_value **argv
			...
			*/
			...
			if( zA && zB ){
				struct compareInfo *pInfo = sqlite3_user_data(fctx);
#ifdef SQLITE_TEST
				sqlite3_like_count++;
#endif

				sqlite3_result_int(fctx, patternCompare(zB, zA, pInfo, escape));
			}
	}
	...
	static void versionFunc(
		...
		sqlite3_value **NotUsed2
			...
			** sqlite3_libversion() C-interface. */
			sqlite3_result_text(fctx, sqlite3_libversion(), -1, SQLITE_STATIC);
	}
	...
	static void sourceidFunc(
		...
		sqlite3_value **NotUsed2
			...
			** sqlite3_sourceid() C interface. */
			sqlite3_result_text(fctx, sqlite3_sourceid(), -1, SQLITE_STATIC);
	}
		...
		sqlite3_value **argv
			...
			*/
			if( (zOptName = (const char*)sqlite3_value_text(argv[0]))!=0 ){
				sqlite3_result_int(fctx, sqlite3_compileoption_used(zOptName));
			}
	}
	...
	*/
	...
	static void compileoptiongetFunc(
		...
		sqlite3_value **argv
			...
			*/
			n = sqlite3_value_int(argv[0]);
			sqlite3_result_text(fctx, sqlite3_compileoption_get(n), -1, SQLITE_STATIC);
	}
#endif /* SQLITE_OMIT_COMPILEOPTION_DIAGS */
	...
	*/
	static void quoteFunc(sqlite3_context *fctx, int argc, sqlite3_value **argv){
		assert( argc==1 );
		UNUSED_PARAMETER(argc);
		switch( sqlite3_value_type(argv[0]) ){
		case SQLITE_FLOAT: {
			...
			sqlite3_result_text(fctx, zBuf, -1, SQLITE_TRANSIENT);
			break;
						   }
		case SQLITE_INTEGER: {
			sqlite3_result_value(fctx, argv[0]);
		...
		case SQLITE_BLOB: {
			...
			zText = (char *)contextMalloc(fctx, (2*(i64)nBlob)+4); 
			if( zText ){
				int i;
				for(i=0; i<nBlob; i++){
				...
				zText[0] = 'X';
				zText[1] = '\'';
				sqlite3_result_text(fctx, zText, -1, SQLITE_TRANSIENT);
				sqlite3_free(zText);
			}
			break;
						  }
		case SQLITE_TEXT: {
			...
			z = contextMalloc(fctx, ((i64)i)+((i64)n)+3);
			if( z ){
				z[0] = '\'';
				...
				z[j++] = '\'';
				z[j] = 0;
				sqlite3_result_text(fctx, z, j, sqlite3_free);
			}
			break;
						  }
		default: {
	...
	*/
	static void unicodeFunc(
		...
		sqlite3_value **argv
			...
			const unsigned char *z = sqlite3_value_text(argv[0]);
			(void)argc;
			if( z && z[0] ) sqlite3_result_int(fctx, sqlite3Utf8Read(&z));
	}
	...
	*/
	static void charFunc(
		...
		sqlite3_value **argv
			...
			sqlite3_result_text(fctx, (char*)z, (int)(zOut-z), sqlite3_free);
	}
	...
	*/
	static void hexFunc(
		...
		sqlite3_value **argv
			...
			z = zHex = contextMalloc(fctx, ((i64)n)*2 + 1);
			if( zHex ){
				for(i=0; i<n; i++, pBlob++){
				...
				*z = 0;
				sqlite3_result_text(fctx, zHex, n*2, sqlite3_free);
			}
	}
		...
		sqlite3_value **argv
			...
			if( zPattern==0 ){
				assert( sqlite3_value_type(argv[1])==SQLITE_NULL
					|| sqlite3_context_db_handle(fctx)->mallocFailed );
				return;
			}
			if( zPattern[0]==0 ){
				assert( sqlite3_value_type(argv[1])!=SQLITE_NULL );
			...
			zOut = contextMalloc(fctx, (i64)nOut);
			if( zOut==0 ){
				return;
			}
			...
			assert( j<=nOut );
			zOut[j] = 0;
			sqlite3_result_text(fctx, (char*)zOut, j, sqlite3_free);
	}
	...
	static void trimFunc(
		...
		sqlite3_value **argv
			...
			if( argc==1 ){
				...
				if( nChar>0 ){
					azChar = contextMalloc(fctx, ((i64)nChar)*(sizeof(char*)+1));
					if( azChar==0 ){
						return;
					}
			...
			if( nChar>0 ){
				flags = SQLITE_PTR_TO_INT(sqlite3_user_data(fctx));
				if( flags & 1 ){
					while( nIn>0 ){
						int len = 0;
						for(i=0; i<nChar; i++){
			...
			sqlite3_result_text(fctx, (char*)zIn, nIn, SQLITE_TRANSIENT);
	}
	...
	static void soundexFunc(
		...
		sqlite3_value **argv
			...
			if( zIn[i] ){
				...
				zResult[j] = 0;
				sqlite3_result_text(fctx, zResult, 4, SQLITE_TRANSIENT);
			}else{
				/* IMP: R-64894-50321 The string "?000" is returned if the argument
				** is NULL or contains no ASCII alphabetic characters. */
				sqlite3_result_text(fctx, "?000", 4, SQLITE_STATIC);
			}
	}
	...
	*/
	static void loadExt(sqlite3_context *fctx, int argc, sqlite3_value **argv){
		...
		if( zFile && sqlite3_load_extension(db, zFile, zProc, &zErrMsg) ){
			sqlite3_result_error(fctx, zErrMsg, -1);
