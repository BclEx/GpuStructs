...
int main(int argc, char **argv)
{
    ...
    {
        g_bQAReadback = true;
        printf("simpleGL - (automated test no-OpenGL)\n");
        runTest(argc, argv, "ref_simpleGL.bin");
    } 
    else 
    {
...
void runAutoTest(int devID, char **argv, char *ref_file)
{
    ...
    sdkDumpBin(imageData, mesh_width*mesh_height*sizeof(float), "simpleGL.bin");
    reference_file = sdkFindFilePath(ref_file, argv[0]);

    if (!sdkCompareBin2BinFloat("simpleGL.bin", reference_file,
