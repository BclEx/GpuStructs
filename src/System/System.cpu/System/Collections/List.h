...
namespace System { namespace Collections {
	...
	inline void * ListArrayNew(int num, bool zeroBuffer)
	{
		T *ptr = nullptr;
		if (zeroBuffer)
	...
	/// </summary>
	template<typename T>
	inline void ListArrayDelete(void *ptr, int num)
	{
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline void *ListArrayResize(void * voldptr, int oldNum, int newNum, bool zeroBuffer)
	{
		T *oldptr = (T *)voldptr;
		T *newptr = nullptr;
		if (newNum > 0)
	...
	/// </summary>
	template<class T>
	inline type *ListNewElement() { return new T; }
	...
	public:
		...
		List<T, Tag> &operator=(const List<T, Tag> &other);
		...
		T &Alloc();										// returns reference to a new data element at the end of the list
		...
		//void Sort(cmp_t *compare = (cmp_t *)&idListSortCompare<T, tag>);
		void SortWithTemplate( const Sort<T> &sort = Sort_QuickDefault<T>());
		...
		operator List<T, _tag> &() { return *reinterpret_cast<List<T, _tag> *>(this); }
		template<memTag_t _tag>
		operator const List<T, _tag> &() const { return *reinterpret_cast<const List<T, _tag> *>(this); }

		//------------------------
		// memTag
		// Changing the memTag when the list has an allocated buffer will result in corruption of the memory statistics.
		//------------------------
		memTag_t GetMemTag() const { return (memTag_t)_memTag; }
		void SetMemTag(memTag_t tag_) { _memTag = (byte)tag_; }
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline List<T, Tag>::List(int newgranularity)
	{
	...
	/// </summary>
	template<typename T, memTag_t Tag >
	inline List<T, Tag>::idList(const List &other)
	{
		_list = nullptr;
		*this = other;
	}
	...
	/// <summary>
	template<typename T, memTag_t Tag>
	inline void List<T, Tag>::DeleteContents(bool clear)
	{
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline size_t List<T, Tag>::Allocated() const { return _size * sizeof(T); }

	/// <summary>
	/// return total size of list in bytes, but doesn't take into account additional memory allocated by _type_
	/// </summary>
	template<typename T, memTag_t Tag>
	inline size_t List<T, Tag>::Size() const { return sizeof(List<T, Tag>) + Allocated(); }

	/// <summary>
	/// MemoryUsed
	/// </summary>
	template<typename T, memTag_t Tag>
	inline size_t List<T, Tag>::MemoryUsed() const { return _num * sizeof(*_list); }
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline int List<T, Tag>::Num() const { return _num; }

	/// <summary>
	/// Returns the number of elements currently allocated for.
	/// </summary>
	template<typename T, memTag_t Tag>
	inline int List<T, Tag>::NumAllocated() const { return _size; }

	/// <summary>
	/// SetNum
	/// </summary>
	template<typename T, memTag_t Tag>
	inline void List<T, Tag>::SetNum(int newnum)
	{
		assert(newnum >= 0);
		if (newnum > _size)
			Resize(newnum);
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline void List<T, Tag>::SetGranularity(int newgranularity)
	{
		assert(newgranularity > 0);
		_granularity = newgranularity;
		if (_list)
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline int List<T, Tag>::GetGranularity() const { return _granularity; }

	/// <summary>
	/// Resizes the array to exactly the number of elements it contains or frees up memory if empty.
	/// </summary>
	template<typename T, memTag_t Tag>
	inline void List<T, Tag>::Condense() {
		if (_list)
			if (_num)
				Resize(_num);
			else
	...
	/// </summary>
	template<typename t, memTag_t Tag>
	inline void List<T, Tag>::Resize(int newsize)
	{
		assert(newsize >= 0);
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline void List<T, Tag>::Resize(int newsize, int newgranularity)
	{
		assert(newsize >= 0);
		assert(newgranularity > 0);
		_granularity = newgranularity;
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline void List<T, Tag>::AssureSize(int newSize)
	{
		int newNum = newSize;
		if (newSize > _size)
		{
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline void idList<T, Tag>::AssureSize(int newSize, const T &initValue)
	{
		int newNum = newSize;
		if (newSize > _size)
		{
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline void List<_type_,_tag_>::AssureSizeAlloc(int newSize, new_t *allocator)
	{
		int newNum = newSize;
		if (newSize > size)
		{
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline List<T, Tag> &List<T, Tag>::operator=(const List<T, Tag> &other)
	{
		Clear();
		_num = other._num;
		_size = other._size;
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline const T &List<_type_, Tag>::operator[](int index) const
	{
		assert(index >= 0);
		assert(index < _num);
		return _list[index];
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline T &List<T, Tag>::operator[](int index)
	{
		assert(index >= 0);
		assert(index < _num);
		return _list[index];
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline T * List<T, Tag>::Ptr() { return _list; }
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	const inline T *List<T, Tag>::Ptr() const { return _list; }

	/// <summary>
	/// Returns a reference to a new data element at the end of the list.
	/// </summary>
	template<typename T, memTag_t Tag>
	inline T &List<T, Tag>::Alloc()
	{
		if (!_list)
			Resize(_granularity);
		if (_num == _size)
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline int List<T, Tag>::Append(T const &obj)
	{
		if (!_list)
			Resize(_granularity);
		if (_num == _size)
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline int List<T, Tag>::Insert(T const &obj, int index)
	{
		if (!_list)
			Resize(_granularity);
		if (_num == _size)
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline int List<T, Tag>::Append(const List<T, Tag> &other)
	{
		if (!_list) 
		{
			// this is a hack to fix our memset classes
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline int List<T, Tag>::AddUnique(T const &obj)
	{
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline int List<T, Tag>::FindIndex(T const &obj) const
	{
		for (int i = 0; i < _num; i++)
			if (list[i] == obj)
				return i;
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline T *List<T, Tag>::Find(T const &obj) const
	{
		int i = FindIndex(obj);
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline int ist<T, Tag>::FindNull() const
	{
		for (int i = 0; i < _num; i++)
			if (_list[i] == nullptr)
				return i;
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline int List<T, Tag>::IndexOf(T const *objptr) const
	{
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline bool List<T, Tag>::RemoveIndex(int index)
	{
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline bool List<T, Tag>::RemoveIndexFast(int index)
	{
		if (index < 0 || index >= _num)
			return false;
		_num--;
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline bool List<T, Tag>::Remove(T const &obj)
	{
		int index = FindIndex(obj);
	...
	/// </summary>
	//template<typename T, memTag_t Tag>
	//inline void List<T, Tag>::Sort(cmp_t *compare)
	//{
	//	if (!_list)
	//		return;
	...
	/// </summary>
	template<typename T, memTag_t Tag>
	inline void List<T, Tag>::SortWithTemplate(const Sort<T> & sort)
	{
		if (_list == nullptr)
			return;
		sort.Sort(Ptr(), Num());
	...
	/// </summary>
	//template<typename T, memTag_t Tag>
	//inline void List<T, Tag>::SortSubSection(int startIndex, int endIndex, cmp_t *compare)
	//{
	//	if (!_list)
	//		return;
	//	if (startIndex < 0)
	...
	T *FindFromGeneric(List<T, Tag> &list, const TCompare &other)
	{
	...
	/// </summary>
	template<typename T, memTag_t Tag, typename TCompare>
	T *FindFromGenericPtr(List<T, Tag> &list, const TCompare & other)
	{
		for (int i = 0; i < list.Num(); i++)
			if (*list[i] == other)
				return &list[i];
