...
namespace System { namespace Collections {
	...
	/// </about>
	template<class T_, int Elements>
	class Array
	{
	public:
		/// <summary>
		...
		/// array operators
		/// </summary>
		const T_ &operator[](int index) const { assert((unsigned)index < (unsigned)Elements); return _ptr[index]; }
		T_ &operator[](int index) { assert((unsigned)index < (unsigned)Elements); return _ptr[index]; }

		/// <summary>
		/// returns a pointer to the list
		/// </summary>
		const T_ *Ptr() const { return _ptr; }
		T_ *Ptr() { return _ptr; }
