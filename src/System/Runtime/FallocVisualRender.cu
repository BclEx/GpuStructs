...
__global__ void RenderHeap(quad4 *b, fallocHeap *heap, unsigned int offset)
{
	int index = offset;
	// heap
	b[index] = make_quad4(
...
int GetRenderQuads(size_t blocks)
{ 
	return 3 + (blocks * 3);
}

void LaunchRender(float4 *b, size_t blocks, fallocHeap* heap)
{
